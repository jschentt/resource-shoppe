...
import { GoodDetail } from '.'
import {
  fetchGoodListByType,
  fetchGoodListByCategoryId,
  fetchGoodListByPromotionId
} from '@/api/goods'
import { GOODS_TYPE } from '../const'
...
const formatData = computed(() => {
  return list.value.filter((it) => !props.exclude.includes(it.id))
})
...
const fetchGoodsList = async () => {
  try {
    let res = null
    if (props.categoryId) {
      res = await fetchGoodListByCategoryId(
        {
          ...pages
        },
        props.categoryId
      ...
      res = await fetchGoodListByPromotionId(
        {
          ...pages
        },
        props.account
      ...
      res = await fetchGoodListByType({
        type: GOODS_TYPE[props.activeName],
        name: props.keywords,
        ...pages
      })
    }
